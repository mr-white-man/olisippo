export declare type AssetSource = AssetSourceCloudinary | AssetSourceBynder | AssetSourceIframe;
export interface AssetSourceCloudinary {
    type: 'cloudinary';
    /**
     * if not specified defaults to 'cloudinary'
     */
    name?: string;
    /**
     * Cloudinary asset preview function
     * If not specified will default to:
     * ({ value }) => {
     *   title: value?.public_id,
     *   image: value?.derived?.[0]?.secure_url ?? value?.secure_url
     * }
     */
    preview?: AssetPreview;
    /**
     * asset transform function, if not specified will use the asset "as is"
     */
    transform?: (options: {
        assetData: any;
    }) => any;
}
export interface AssetSourceBynder {
    type: 'bynder';
    /**
     * if not specified defaults to 'bynder'
     */
    name?: string;
    /**
     * Bynder asset preview function
     * If not specified will default to:
     * ({ value }) => {
     *   title: value?.public_id,
     *   image: value?.derived?.[0]?.secure_url ?? value?.secure_url
     * }
     */
    preview?: AssetPreview;
    /**
     * asset transform function, if not specified will use the asset "as is"
     */
    transform?: (options: {
        assetData: any;
    }) => any;
}
export interface AssetSourceIframe {
    type: 'iframe';
    name: string;
    url: string;
    preview: AssetPreview;
    /**
     * asset transform function, if not specified will use the asset "as is"
     */
    transform?: (options: {
        assetData: any;
    }) => any;
}
export declare type AssetPreview = AssetPreviewFunction | AssetPreviewFields;
export declare type AssetPreviewFunction = (options: {
    assetData: any;
}) => AssetPreviewFields;
export interface AssetPreviewFields {
    title?: string;
    image: string;
}
//# sourceMappingURL=asset-source.d.ts.map