...
export interface StackbitConfig {
     ...
     */
    onContentCreate?: (options: OnContentCreateOptions) => Record<string, any>;
...
export declare type Assets = StaticAssets | RelativeAssets;
...
export declare type SiteMapEntry = SiteMapBaseEntry | SiteMapDocumentEntry;
...
export interface OnContentCreateOptions extends ConfigDelegate {
    /**
     * This object contains data for creating one or more new documents.
     * The data includes values for the document's primitive fields, nested
     * objects and arrays, as well as instructions to set reference fields to
     * existing documents or create additional documents and set them to the
     * reference fields.
     *
     * In contrast to other methods that work with the {@link Document} and
     * {@link DocumentField} types, this object has a simple structure that
     * maps field names to their corresponding values. Some complex field types
     * have "instruction" properties that specify how these fields should be
     * created.
     *
     * For example, to create a new nested object for the 'model' field type,
     * the field value should be an object with a `$$type` property specifying
     * the model name of the new nested object, and additional optional
     * properties specifying the field values to populate the new nested object.
     *
     * Similarly, to create a new document and link it to a 'reference' field,
     * the field value should be an object with a `$$type` property specifying
     * the model name of the new document, and additional optional properties
     * specifying the field values to populate the new document.
     *
     * To link the 'image' and 'reference' fields to existing entities, the
     * field value should be an object with a `$$ref` property specifying the
     * entity ID.
     *
     * If a model field is not specified in the data, it will be set to its
     * default value. If the default value is not specified in the model field,
